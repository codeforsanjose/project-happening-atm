...
const subscribeDisabled = (item) =>
	item.status === MeetingItemStates.IN_PROGRESS || item.status === MeetingItemStates.COMPLETED;
...
const RenderedAgendaItem = forwardRef(({ handleSelection, isSelected, item, id, ...props }, ref) => {
	...
	return (
		<div {...props} ref={ref} className="AgendaItem">
			{item.status !== MeetingItemStates.PENDING && <div className="item-status">{item.status}</div>}
			...
			<div className="item-links">
				<Link to="/" className={classnames({ disabled: subscribeDisabled(item) })} onClick={handleSubmit}>
					<div className="link">
						<NotificationsIcon />
						{loading && <Spinner />}
						<p>
