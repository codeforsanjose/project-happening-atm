...
module.exports = (logger) => {
  ...
  const resolverHandler = async (resolverFunc, args, context) => {
    ...
    try {
      ...
      logger.info(
        'Initiating DB Client - backend/graphql_api/graphql/apolloServer.js'
      );
      await dbClient.init();
      ...
      try {
        logger.info(
          'Ending DB connection - backend/graphql_api/graphql/apolloServer.js'
        );
        await dbClient.end();
      } catch (err) {
  ...
  const resolvers = {
    Query: {
      ...
      getAllUpcomingMeetings: async () => {
        logger.info('Initiating GetAllMeetings Query resolver');
        return resolverHandler(queryResolver.getAllUpcomingMeetings);
      },
